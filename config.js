...
//
global.prefa = ["#"]; 
global.prefix = ["#"];                                    //Default prefix here. you can change if you want.
prefix = ["#"];  
...
global.mess = {
  ...
  prem : 'Tut mir leid, aber dieser befehl kann nur von Premium-Benutzern verwendet werden.',
  useradmin: '*This command can only be used by an admin.* !',
  botadmin: 'Sorry leider kann ich diesen Befehl nicht ausführen, ohne ein *Admin* dieser Gruppe zu sein.',
  botowner: 'This command can only be used by my owners.',
  ...
  nonreg: 'You are not registered! Please register first via private chat with #reg or #register.',
  bangc: 'This group is restricted from using commands! Kindly contact an owner for assistance.',
...
global.rolee = function rolee(level) {
  ...
  const rolee = [
    ...
    { name: 'Hacker', level: 1100 }, { name: 'Owner', level: 10000 }, { name: 'Phoenix-Bot', level: 100000 }, 
  ];

  return rolee.reverse().find(userr => level >= userr.level)
};
